...
export function SchedulingModal({ goalId, onClose, isRescheduling = false }: SchedulingModalProps) {
  ...
  const handleViewCalendar = () => {
    onClose();
    // Navigate to timeline - you might want to use router here
    window.location.href = '/timeline';
  };

  return createPortal(
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[9999] p-4">
      <div className="bg-[#0D1B3D] border border-[#00E0FF]/20 rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#00E0FF]/10">
          <div className="flex items-center gap-3">
            <Calendar className="w-6 h-6 text-[#00E0FF]" />
        ...
        <div className="p-6">
          {step === "intro" && (
            <div className="text-center">
              <div className="mb-6">
                <MessageCircle className="w-16 h-16 text-[#00E0FF] mx-auto mb-4" />
              ...
              <div className="flex gap-3 justify-center">
                <button
                  ...
                  className="px-6 py-3 bg-[#00E0FF] text-[#0D1B3D] rounded-lg font-medium hover:bg-[#00E0FF]/90 transition-colors"
                >
                ...
                {!isRescheduling && (
                  <button
                    onClick={handleSkip}
                    className="px-6 py-3 bg-transparent border border-[#00E0FF]/30 text-[#00E0FF] rounded-lg font-medium hover:bg-[#00E0FF]/10 transition-colors"
                  >
                    Schedule Later
                  </button>
                )}
          ...
          {step === "chat" && (
            <div>
              ...
              <div className="bg-[#142850]/60 border border-[#00E0FF]/10 rounded-lg p-4 mb-4 max-h-60 overflow-y-auto">
                    ...
                    <div
                      ...
                      <div
                        className={`px-4 py-3 rounded-lg max-w-[80%] ${
                          message.role === 'user'
                            ? 'bg-[#00E0FF] text-[#0D1B3D] font-medium'
                            : 'bg-[#142850]/80 text-gray-200 border border-[#00E0FF]/10'
                        }`}>
                        {message.role === 'user' ? (
                          message.content
                        ) : (
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-3 bg-[#142850]/60 border border-[#00E0FF]/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#00E0FF]/50"
                  placeholder="Type your availability..."
                  disabled={isSaving}
                />
                <button
                  onClick={() => handleSendMessage()}
                  className="px-6 py-3 bg-[#00E0FF] text-[#0D1B3D] rounded-lg font-medium hover:bg-[#00E0FF]/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={isSaving}>
